    sealed trait Maybe[+A] {
      ...
      def equalTo[AA >: A](other: Maybe[AA])(implicit aeq: (AA, AA) => Boolean): Boolean = cata(aa => if (other.isDefined) aeq(aa, other.get) else false, other.isEmpty)
    }
    ...
    object monads {
      ...
      implicit def ReaderMonad[I]: Monad[({ type λ[α] = I => α })#λ] = new Monad[({ type λ[α] = I => α })#λ] {
        ...
        def bind[A, B](f: A => (I => B)): (I => A) => (I => B) = bind(f) 
      }
    }
